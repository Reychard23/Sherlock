...
async def process_files_endpoint():
    ...
    required_cols = ["Archivo", "Sheet", "Columna", "Nombre unificado", "Acción"]
    ...
    no_index = { archivo for archivo in (uploaded & expected) if archivo not in indexed }
    ...
    for (archivo, hoja), mapeo in rename_dict.items():
        ...
        try:
            ...
            # Drop de columnas
            to_drop = [col for (a,h,col) in drop_set if a==archivo and h==hoja]
            # Solo intentar dropear si hay columnas para dropear Y si existen en el DF
            cols_to_actually_drop = [col for col in to_drop if col in df.columns]
            if cols_to_actually_drop:
                 df = df.drop(columns=cols_to_actually_drop) # Quitamos errors="ignore" para ser más estrictos si la columna a dropear del índice no existe en el archivo/hoja

            # Rename de columnas keep
            # Crear un mapeo que solo contenga columnas que realmente existen en el DF
            actual_mapeo = {orig_col: new_col for orig_col, new_col in mapeo.items() if orig_col in df.columns}
            if actual_mapeo:
...
async def slack_command(request: Request):
    ...
    user_question = form_data.get("text") # El texto de la pregunta debería estar en el campo 'text'
